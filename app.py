...
def result():
    ...
    if len(result_representation) == 0:
        result_representation = ['visual_stats']
    ...
    final_list, min_date, max_date, min_age, max_age, min_bmi, max_bmi = main_func.assessment_rules_function(rule_list,
                                                                                                             start_age,
                                                                                                             end_age,
                                                                                                             start_bmi,
                                                                                                             end_bmi,
    ...
    if start_date == "" and end_date == "":
        ...
        if ((start_date < min_date and end_date > max_date) or (
                start_date > max_date and end_date > max_date) or (start_date < min_date and end_date < min_date) or
                (start_date > max_date and end_date < min_date)):
            start_date = min_date
            end_date = max_date
            warn = "(Input date is out of actual range, default range is considered)"
        elif end_date > max_date or end_date < min_date:
            end_date = max_date
            warn = "(End date is out of range. Default end date considered)"
    ...
    if start_bmi == "" and end_bmi == "":
        start_bmi = round(min_bmi, 2)
        end_bmi = round(max_bmi, 2)

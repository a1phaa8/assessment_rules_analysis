...
from .activity_code import *
from .cardiac_index import cardiac_index
from .cvreactivity_features_optimization import *
from .dataframe_processing_functions import *

json_folder = "input/json_folder"


def process_common_df(rule_names, df_patient, cohort_all, common_params, trend_window, gender, s_age, e_age):
    ...
    for rule_name in rule_names:
        ...
        if rule_name in ['ruleset6_V4_combinatorial_rules_PulsePressure_SBP_rule', 'ruleset4_V3_1_extreme_SBP_rule',
            ...
            if bp_flag == 0:
                ...
                processed_dataframes["sbp_df"] = [processed_sbp_df, min_age, max_age, min_bmi, max_bmi, no_rows]
                ...
                processed_dataframes["dbp_df"] = [processed_dbp_df, min_age, max_age, min_bmi, max_bmi, no_rows]
                bp_flag = 1
            if rule_name in ['ruleset6_V4_combinatorial_rules_PulsePressure_SBP_rule']:
                ...
                merged_dataframe['PP_obs'] = merged_dataframe.apply(subtract_lists, axis=1)
                merged_dataframe['Pulse Pressure'] = merged_dataframe['PP_obs'].apply(lambda x: x[-1] if x else None)
            ...
            if rule_name in ['ruleset5_V3_1_progression_sbp_rule']:
                ...
                processed_dataframes['sbp_progression_df'] = [df_sbp_prog, min_age, max_age, min_bmi, max_bmi, no_rows]
            if rule_name in ['ruleset5_V3_1_progression_dbp_rule']:
                ...
                df_dbp_prog = df_dbp_prog.apply(
                    lambda x: extract_format_find_trend(x, trend_window), axis=1)
                processed_dataframes['dbp_progression_df'] = [df_dbp_prog, min_age, max_age, min_bmi, max_bmi, no_rows]
            if rule_name in ['ruleset6_V3_1_combinatorial_rules_SBP_DBP_rule']:
                merged_sbp_dbp = pd.merge(
                    ...
                    on='subject_reference', how='inner')
                processed_dataframes['sbp_dbp_merged'] = [merged_sbp_dbp, min_age, max_age, min_bmi, max_bmi, no_rows]
        if rule_name in ['ruleset6_V3_1_combinatorial_hr_hrv_rule',
            ...
            if rule_name in ['ruleset6_V4_combinatorial_rules_target_HR_rule']:
                ...
                processed_target_hr_dataframe, min_date, max_date, min_age, max_age, min_bmi, max_bmi, no_rows = process(
                    hr_dataframe,
                    hr_code,
                    df_patient,
                    cohort_all,
            ...
            if hr_flag == 0:
                ...
                with open(hr_json_file_path, 'r') as json_file:
                    hr_data = json.load(json_file)
                    hr_dataframe = pd.DataFrame(hr_data)
                processed_hr_df, min_date, max_date, min_age, max_age, min_bmi, max_bmi, no_rows = process(
                    hr_dataframe,
                    hr_code,
                    df_patient,
                    cohort_all,
                ...
                processed_dataframes["heartrate_df"] = [processed_hr_df, min_age, max_age, min_bmi, max_bmi, no_rows]
                hr_flag = 1
            if rule_name in ['ruleset5_V3_1_progression_rhr_rule']:
                ...
                df_heartrate = df_heartrate.apply(
                    lambda x: extract_format_find_trend(x, trend_window), axis=1)
                processed_dataframes['rhr_progression_df'] = [df_heartrate, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset6_V3_1_combinatorial_hr_hrv_rule', 'ruleset4_V2_1_extreme_Heart_Rate_Variability_rule',
            ...
            if hrv_flag == 0:
                ...
                processed_dataframes["hrv_df"] = [processed_hrv_df, min_age, max_age, min_bmi, max_bmi, no_rows]
                hrv_flag = 1
            if rule_name in ['ruleset5_V3_1_progression_hrv_rule']:
                ...
                df_hrv = df_hrv.apply(
                    lambda x: extract_format_find_trend(x, trend_window), axis=1)
                processed_dataframes['hrv_progression_df'] = [df_hrv, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset6_V3_1_combinatorial_hr_hrv_rule']:
            merged_hr_hrv = pd.merge(
                ...
                on='subject_reference', how='inner')
            processed_dataframes['hr_hrv_merged'] = [merged_hr_hrv, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset4_V2_1_extreme_PRQ_rule',
            ...
            if prq_flag == 0:
                ...
                processed_dataframes["prq_df"] = [processed_prq_df, min_age, max_age, min_bmi, max_bmi, no_rows]
                prq_flag = 1
            if rule_name in ['ruleset5_V4_progression_prq_rule']:
                ...
                df_prq = df_prq.apply(
                    lambda x: extract_format_find_trend(x, trend_window), axis=1)
                processed_dataframes['prq_progression_df'] = [df_prq, min_age, max_age, min_bmi, max_bmi, no_rows]
        if rule_name in ['ruleset2_V4_risk_clinical_risk_score_rule', 'ruleset2_V2_1_cad_risk_assessment_rule']:
            if cad_flag == 0:
                ...
                processed_dataframes["cad_df"] = [processed_cad_df, min_age, max_age, min_bmi, max_bmi, no_rows]
                cad_flag = 1
        if rule_name in ['ruleset1_V2_1_Fitness_VO2max_Fatness_BMI_rule']:
            ...
            ## check for lowest value between both dataframes/parameters
            no_rows = no_rows_1 or no_rows
            min_age = min(min_age, min_age_1)
            max_age = max(max_age, max_age_1)
            min_bmi = min(min_bmi, min_bmi_1)
            max_bmi = max(max_bmi, max_bmi_1)
            ...
            merged_df = add_grade_column(merged_df, "cadphr-vo2maxra")
            merged_df['cadphr-vo2maxra_grade'] = merged_df['cadphr-vo2maxra_grade'].astype(int)
            processed_dataframes["vo2max_df"] = [merged_df, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset3_V4_emotional_influx_Stress_Disorder_rule']:
            ...
            processed_si_dataframe, min_date, max_date, min_age, max_age, min_bmi, max_bmi, no_rows = process(
                ehi_dataframe_si,
                "",
                df_patient,
                cohort_all,
            ...
            processed_ps_dataframe, min_date, max_date, min_age, max_age, min_bmi, max_bmi, no_rows = process(
                ehi_dataframe_ps,
                "",
                df_patient,
                cohort_all,
            ...
            merged_stress = pd.merge(processed_si_dataframe, processed_ps_dataframe[
                ['subject_reference', 'pss_value', 'cadphr-pss4_grade']], on='subject_reference',
                                     how='inner')
            processed_dataframes['stress_disorder_df'] = [merged_stress, min_age, max_age, min_bmi, max_bmi, no_rows]
        if rule_name in ['ruleset1_V2_1_Heart_Rate_Recovery_rule']:
            ...
            processed_dataframes['hrr_df'] = [processed_hrrra_dataframe, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset2_V2_1_osa_risk_assessment_rule']:
            ...
            processed_dataframes['osa_df'] = [processed_osa_dataframe, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset2_V2_1_diabetes_risk_assessment_rule']:
            ...
            processed_dataframes['drs_df'] = [processed_drs_dataframe, min_age, max_age, min_bmi, max_bmi, no_rows]
        ...
        if rule_name in ['ruleset5_V4_progression_resprate_rule']:
            ...
            processed_rr_dataframe, min_date, max_date, min_age, max_age, min_bmi, max_bmi, no_rows = process(
                ehi_dataframe_rr,
                '9303-9',
                df_patient,
                cohort_all,
        ...
        if rule_name in ['ruleset2_V4_risk_clinical_risk_score_rule']:
            ...
            processed_cad_Hage_dataframe, min_date, max_date, min_age, max_age, min_bmi, max_bmi, no_rows = process(
            ...
            processed_cad_dataframe.rename(columns={'Latest': 'cadphr-cadrisk10_value'}, inplace=True)
            merged_df = pd.merge(processed_cad_Hage_dataframe[['subject_reference', 'cadphr-rcvage_value']],
                                 ...
                                 on='subject_reference', how='inner')
            processed_dataframes['cad_heartage_df'] = [merged_df, min_age, max_age, min_bmi, max_bmi, no_rows]
        if rule_name in ['ruleset1_V4_cardio_respiratory_fitness_rule']:
            ...
            print("size of cardio df after merging:", df_final.shape[0])
            if df_final.shape[0] == 0:
                no_rows = 1
                df_final = pd.merge(df_final_copy, cohort_all, on='subject_reference', how='inner')
            processed_dataframes['cvreactivity'] = [df_final, min_age, max_age, min_bmi, max_bmi, no_rows]

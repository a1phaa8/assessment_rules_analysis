...
gender = data["gender"]
age_lower_limit, age_upper_limit = data["start_age"], data["end_age"]
BMI_lower_limit, BMI_upper_limit = data["BMI_start"], data["BMI_end"]
des_opt = data["display_options"]
rule_name = data["rule_name_list"]
...
if age_lower_limit:
    age_lower_limit = int(age_lower_limit)
if age_upper_limit:
    age_upper_limit = int(age_upper_limit)
if BMI_lower_limit:
...
def compare_age(start_age, end_age, min_age, max_age):
    if start_age and end_age:
        if start_age > max_age or end_age < min_age:
            print("WARNING\n")
            start_age = min_age
            end_age = max_age
    if start_age == "" and end_age == "":
...
def assessment_rules_function(criteria_name, age_lower_limit, age_upper_limit, bmi_lower, bmi_upper, start_d, end_d,
                              gender, display_option):
    output_list = []
    required_dataframes, min_date, max_date, min_age, max_age, min_bmi, max_bmi = process_data(criteria_name,
                                                                                               age_lower_limit,
                                                                                               age_upper_limit, start_d,
                                                                                               end_d, bmi_lower,
                                                                                               bmi_upper, gender)
    # if age_lower_limit and age_upper_limit:
    #     if age_lower_limit > max_age or age_upper_limit < min_age:
    #         print("WARNING\n")
    #         age_lower_limit = min_age
    #         age_upper_limit = max_age

    age_lower_limit, age_upper_limit = compare_age(age_lower_limit, age_upper_limit, min_age, max_age)
    start_d, end_d = compare_dates(start_d, end_d, min_date, max_date)
    ...
    if ('descriptive_stats' in display_option) & ('visual_stats' in display_option):
        for rule_name, dataframe in required_dataframes.items():
            ...
            KF_pie = ""
            unique_KF_bar = ""
            unique_bar = assessment_rule_unique_criteria_bar(dataframe, rule_name, age_lower_limit, age_upper_limit,
                                                             start_d,
                                                             end_d, gender)
            unique_pie = assessment_rule_unique_criteria_pie_chart(dataframe, rule_name, age_lower_limit,
                                                                   age_upper_limit, start_d,
                                                                   end_d, gender)
            grouped_bar = assessment_rule_grouped_criteria_bar(dataframe, rule_name, age_lower_limit, age_upper_limit,
                                                               start_d,
                                                               end_d, gender)
            grouped_pie = assessment_rule_grouped_criteria_pie_chart(dataframe, rule_name, age_lower_limit,
                                                                     age_upper_limit, start_d,
                                                                     end_d,
                                                                     gender)
            ...
            if rule_name in ['ruleset6_V4_combinatorial_rules_PulsePressure_SBP_rule',
                ...
                KF_bar = assessment_rule_KF_criteria_bar(dataframe, rule_name, age_lower_limit, age_upper_limit,
                                                         start_d,
                                                         end_d,
                                                         gender)
                KF_pie = assessment_rule_KF_criteria_pie_chart(dataframe, rule_name, age_lower_limit, age_upper_limit,
                                                               start_d,
                                                               end_d, gender)
                unique_KF_bar = assessment_rule_unique_KF_criteria_bar(dataframe, rule_name, age_lower_limit,
                                                                       age_upper_limit, start_d,
                                                                       end_d, gender)
            else:
                print("Key Finding doesn't exist for", rule_name)
            descriptive_unique_rule = assessment_unique_description(dataframe, rule_name)
            descriptive_grouped_rule = assessment_grouped_description(dataframe, rule_name)
        ...
        return output_list, min_date, max_date, min_age, max_age, min_bmi, max_bmi
